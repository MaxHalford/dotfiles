...
def find_stash(repo, marker):
    entries = run(["git", "stash", "list", "--format=%gd%x00%H%x00%gs"], cwd=repo)
...
def worktree_for_branch(repo, branch):
    trees = parse_worktrees(run(["git", "worktree", "list", "--porcelain", "-z"], cwd=repo))
    return next((tree.path for tree in trees if tree.branch == branch and not tree.unavailable), None)
...
def publish_from_main(tree, branch_name, message, operation=None):
    ...
    base = run(["git", "rev-parse", "HEAD"], cwd=repo, operation=operation).strip()
    marker = f"shipr-{uuid.uuid4().hex}"
    new_path = None
    transferred = False
    try:
        if operation:
            operation.stage("Moving current changes into a new worktree")
        run(["git", "stash", "push", "--include-untracked", "--message", marker],
            cwd=repo, timeout=120, operation=operation)
        stash = find_stash(repo, marker)
        if stash is None:
            raise CommandError("Could not find the saved changes; inspect Git stash before retrying")
        if operation:
            operation.stage(f"Creating Herdr worktree for {branch}")
        run([os.environ.get("HERDR_BIN_PATH", "herdr"), "worktree", "create",
             *herdr_source(repo), "--branch", branch, "--base", base, "--no-focus"],
            cwd=repo, timeout=120, operation=operation)
        new_path = worktree_for_branch(repo, branch)
        if new_path is None:
            raise CommandError(f"Herdr did not create a worktree for {branch}")
        if operation:
            operation.stage("Restoring changes in the new worktree")
        run(["git", "stash", "apply", "--index", stash[1]], cwd=new_path,
            timeout=120, operation=operation)
        transferred = True
        run(["git", "stash", "drop", stash[0]], cwd=repo)
        result = publish(Worktree(new_path, branch), message, True, operation)
        open_herdr_worktree(repo, new_path, branch)
        return result
    except Exception as error:
        stash = find_stash(repo, marker)
        if new_path is None:
            new_path = worktree_for_branch(repo, branch)
        if stash and new_path is None:
            try:
                run(["git", "stash", "apply", "--index", stash[1]], cwd=repo)
                run(["git", "stash", "drop", stash[0]], cwd=repo)
            except CommandError as restore_error:
                raise CommandError(f"{error}; original changes remain in {stash[0]}: {restore_error}") from error
        elif stash and new_path is not None and not transferred:
            raise CommandError(f"{error}; changes remain in {stash[0]} and worktree {new_path}") from error
        elif new_path is not None and transferred:
            raise CommandError(f"{error}; changes are in worktree {new_path}") from error
        raise
